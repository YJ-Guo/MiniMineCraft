...
private:
    ...
    std::list<int64_t> chunksWaitlist;



    std::mutex mutex_chunks;
    std::mutex mutex_waitList;
    ...
    std::unordered_set<int64_t> m_ChunksWithoutVBO;

    std::list<int64_t> m_ChunksWithVBO;

    std::mutex mut_withoutVBO;
    std::mutex mut_withVBO;
...
//    const uPtr<Chunk>& getChunkAt(int x, int z) const;
    ...
    void drawTrans(ShaderProgram *shaderProgram);
    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.
    void CreateTestScene();

    // Function to whether adding a new chunk based in player's position
    void UpdateTerrain(const glm::vec3 &playerPos);
    void UpdateNeighborChunks(int x, int z);
//    void createLandscape(int x, int z);
    void createBlockTypeWorker(int x, int z);
    void createVBOWorkers(int64_t key);
    void passVBO(int64_t key);
    void blockTypeWorker(int x, int y);
    void VBOWorker(int64_t key);
    void join();
    void addToWaitList(int64_t);

    void UpdateTerrainWithHightMap(QImage *HightMap);
    void updateEdgeWithHightMap(int x1, int z1, int x2, int z2);
    void blockTypeWorkerIMG(int x, int z, int rx, int rz, QImage* img, bool isColor);
    void createBlockTypeWorkerIMG(int x, int z, int rx, int rz, QImage* img, bool isColor);
};
