...
//    Texture *m_textures;
    ...
    FrameBuffer m_fbuffer;  // frame buffer used in post processing

    int m_time;         // A time counter for texture animation
...
public:
    ...
    // Calls Terrain::draw().
    void renderTerrain();
    bool gridMarch(glm::vec3 rayOrigin, glm::vec3 rayDirection, Terrain *terrain, float *out_dist,
                   glm::ivec3 *out_blockHit, glm::ivec3 *out_incidentBlock);

    void UpdateTerrainWithHightMap(QImage *HightMap, bool isColor);
protected:
